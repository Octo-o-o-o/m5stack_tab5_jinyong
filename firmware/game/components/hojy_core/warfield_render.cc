...
void Warfield::render() {
    ...
    if (drawDirty_) {
        /* Rebuilt on every animation step during Acting; allocating and zeroing
         * a fresh 4096-entry vector each time is pure churn. Render runs only
         * on the game thread. */
        static std::vector<const std::string *> effectOverlay;
        effectOverlay.assign(cellInfo_.size(), nullptr);
        drawDirty_ = false;
        ...
        if (acting && ch && effectTexIdx_ >= 0
            ...
            if (skillInfo == nullptr || skillInfo->attackAreaType == 0) {
                ...
                case 1: {
                    const int sx = cameraX_, sy = cameraY_, st = sy * static_cast<int>(mw);
