...
/* Overlay: log TTF load success/failure so a missing font is visible. */
...
Renderer::Renderer(void *win, int w, int h):
    ...
    for (const auto &f: core::config.fonts()) {
        if (ttf_->add(f)) {
            ESP_LOGI(TAB5_TAG, "TTF loaded %s size=%d", f.c_str(), fontSize);
        } else {
            ESP_LOGE(TAB5_TAG, "TTF add failed %s", f.c_str());
        }
    }
}
