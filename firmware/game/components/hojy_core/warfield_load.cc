...
/*
 * Upstream loads every FIGHT###.GRP in the Warfield constructor. For this data
 * set that is 92 files / 6.1 MB / ~5000 records read from the SD card at the
 * first battle, plus 18 "file not found" attempts, and the whole 6 MB then
 * stays in PSRAM for the rest of the session. docs/ARCHITECTURE.md already
 * says resources are loaded on demand.
 *
 * Load a character's animation set the first time that character enters a
 * battle instead. Attempts are remembered so missing files are probed once.
 * Warfield is a Window-owned singleton, so file scope is the right lifetime.
 */
std::set<int> gFightTexTried;

void ensureFightTex(std::vector<std::vector<std::string>> &fightTexData, int headId) {
    if (headId < 0 || static_cast<size_t>(headId) >= fightTexData.size()) { return; }
    if (!gFightTexTried.insert(headId).second) { return; }
    ::hojy::content::GrpData::loadData(fmt::format("FIGHT{:03}.IDX", headId),
                                       fmt::format("FIGHT{:03}.GRP", headId),
                                       fightTexData[headId]);
}
...
bool Warfield::load(std::int16_t warId) {
    ...
    /* Only this battle's participants need animation frames. Keeping every set
     * ever loaded walks back up to the 6.1 MB the eager constructor used to
     * cost; re-reading a few hundred KB per battle is the better trade. */
    for (auto &set : fightTexData_) {
        if (!set.empty()) { set.clear(); set.shrink_to_fit(); }
    }
    gFightTexTried.clear();
...
bool Warfield::putChars(const std::vector<std::int16_t> &chars) {
    ...
    try {
        ...
        for (const auto &ci: nextChars) {
            ensureFightTex(fightTexData_, ci.info.headId);
        }
