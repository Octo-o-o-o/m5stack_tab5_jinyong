...
/* Overlay: refuse empty SDX/SMP headers instead of reading texData_[0]. */
...
bool SubMap::load(std::int16_t subMapId) {
    ...
    if (texData_.empty() || texData_[0].size() < sizeof(uint16_t) * 4) {
        ESP_LOGE(TAB5_TAG, "SMP tex header missing id=%d entries=%u",
                 (int)subMapId, (unsigned)texData_.size());
        return false;
    }
    {
...
void SubMap::releaseTiles() {
    const auto entries = texData_.size();
    texData_.clear();
    texData_.shrink_to_fit();
    subMapLoaded_.clear();
    /* mainCharTex_ points into the atlas that clear() is about to delete. */
    mainCharTex_ = nullptr;
    textureMgr_.clear();
    cellInfo_.clear();
    cellInfo_.shrink_to_fit();
    subMapId_ = -1;
    drawDirty_ = true;
    ESP_LOGI(TAB5_TAG, "SubMap tiles released (%u entries)", (unsigned)entries);
    tab5_log_memory("after_submap_release");
}
...
void SubMap::render() {
    ...
    /* Nothing to draw while the world map owns the memory (releaseTiles). */
    if (cellInfo_.empty() || texData_.empty()) {
...
bool SubMap::tryMove(int x, int y, bool checkEvent) {
    if (subMapId_ < 0 || cellInfo_.empty()) { return true; }
    auto pos = y * mapWidth_ + x;
    if (pos < 0 || static_cast<std::size_t>(pos) >= cellInfo_.size()) { return true; }
    auto &ci = cellInfo_[pos];
...
void SubMap::frameUpdate() {
    ...
    /* Idle NPC tex cycling dirties the whole 320x240 RLE terrain ~15Hz.
     * Skip it on Tab5 so walking stays a single rebuild. Event scripts
     * still poke frames through setCellTexture. */
}
