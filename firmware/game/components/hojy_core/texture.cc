...
/*
 * Overlay of HeroesOfJinYong src/scene/texture.cc for ESP32-P4.
 * Only change: the TextureMgr atlas page size.
 *
 * Upstream packs sprites into 1024x1024 ARGB pages, i.e. 4.19 MB of contiguous
 * PSRAM per page. Every TextureMgr gets its own: head portraits, the title
 * strip, the clouds, and the per-map character sprites -- four to five live
 * pages holding maybe 1.6 MB of actual pixels. That is what leaves no
 * contiguous hole for GlobalMap later.
 *
 * The largest sprite that ever reaches this packer in this data set is 240 px
 * (ENDWORD; MMAP 216, EFT 214, SMP 189, CLOUD 142, TITLE 91, HDGRP 58), so a
 * 512 page keeps a wide margin and cuts each allocation to 1.05 MB.
 */
...
/* Must stay >= the widest/tallest sprite any TextureMgr is asked to pack. */
static constexpr int kAtlasSize = 512;

/* Textures are the largest single class of PSRAM use on this device and they
 * are invisible to heap_caps summaries taken between frames. Keep a running
 * total so the log says how much of PSRAM is pixels. */
static std::size_t gTextureBytes = 0;

std::size_t textureBytesTotal() { return gTextureBytes; }

static void textureAccount(int w, int h, int sign) {
    const std::size_t bytes = (std::size_t)w * (std::size_t)h * 4u;
    if (sign > 0) { gTextureBytes += bytes; }
    else if (gTextureBytes >= bytes) { gTextureBytes -= bytes; }
}
...
Texture *Texture::createAsTarget(Renderer *renderer, int w, int h) {
    ...
    tex->width_ = w;
    tex->height_ = h;
    if (texture) { textureAccount(w, h, 1); }
    return tex;
}
...
Texture::~Texture() {
    if (data_) {
        textureAccount(width_, height_, -1);
...
Texture *Texture::loadFromRLE(Renderer *renderer, const std::string &data, const ColorPalette &palette) {
    ...
    std::uint32_t *pixels;
    int pitch;
    /* create() hands back a Texture whose pixels failed to allocate rather than
     * nullptr, and a failed lock leaves pixels/pitch untouched -- writing the
     * sprite through them was a store to an uninitialised pointer, which on
     * this device means a random reboot exactly when memory is tight. */
    if (SDL_LockTexture(static_cast<SDL_Texture*>(tex->data()), nullptr,
                        reinterpret_cast<void**>(&pixels), &pitch) != 0) {
        ESP_LOGE(TAB5_TAG, "RLE texture lock failed %dx%d", (int)w, (int)h);
        delete tex;
        return nullptr;
    }
    pitch /= sizeof(std::uint32_t);
...
Texture *Texture::loadFromRAW(Renderer *renderer, const std::string &data, int width, int height, const ColorPalette &palette) {
    ...
    if (SDL_LockTexture(static_cast<SDL_Texture*>(tex->data()), nullptr,
                        reinterpret_cast<void**>(&pixels), &pitch) != 0) {
        ESP_LOGE(TAB5_TAG, "RAW texture lock failed %dx%d", width, height);
        delete tex;
        return nullptr;
    }
    pitch /= sizeof(std::uint32_t);
...
TextureMgr::TextureMgr(): rectPacker_(new RectPacker(kAtlasSize, kAtlasSize)) {
}
...
Texture *TextureMgr::loadFromRLE(const std::string &data, std::int16_t index) {
    ...
    if (rpidx < 0) {
        /* Silently returning nullptr here is an invisible sprite. Say which
         * one, so a too-small atlas page is diagnosable instead of mysterious. */
        ESP_LOGE(TAB5_TAG, "atlas pack failed id=%d %dx%d (page %d)",
                 (int)index, (int)w, (int)h, kAtlasSize);
        return nullptr;
    }
    ...
    if (tex == nullptr) {
        ...
        /* Same contract as everywhere else in this port: a Texture whose pixel
         * buffer failed to allocate is not usable, and the slice built below
         * would point into nothing. */
        if (!tex || !tex->data()) {
            ESP_LOGE(TAB5_TAG, "atlas page %d create failed %dx%d",
                     (int)rpidx, kAtlasSize, kAtlasSize);
            delete tex;
            return nullptr;
        }
        tex->enableBlendMode(true);
        textureContainers_[rpidx] = tex;
        ESP_LOGI(TAB5_TAG, "atlas page %d %dx%d; textures now %u KB",
                 (int)rpidx, kAtlasSize, kAtlasSize,
                 (unsigned)(gTextureBytes / 1024u));
    }
    int pitch;
