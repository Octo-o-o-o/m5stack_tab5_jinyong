...
/*
 * Overlay of HeroesOfJinYong src/scene/map.cc for ESP32-P4.
 * Only change: do not allocate miniPanelTex_ when the mini panel is disabled.
 */
...
Map::Map(Renderer *renderer, int x, int y, int width, int height, std::pair<int, int> scale): Node(renderer, x, y, width, height),
    ...
    /* 256x256 ARGB = 256 KB per map, built even when show_map_mini_panel is
     * off. showMiniPanel() already gates every use of it. */
    miniPanelTex_(core::config.showMapMiniPanel()
                      ? Texture::createAsTarget(renderer_, 256, 256) : nullptr),
    ...
    if (miniPanelTex_) { miniPanelTex_->enableBlendMode(true); }
...
void Map::showMiniPanel() {
    ...
    if (core::config.showMapMiniPanel() && miniPanelTex_ != nullptr) {
        if (miniPanelDirty_) {
...
const Texture *Map::getOrLoadTexture(std::int16_t id) {
    ...
    if (tex) { return tex; }
    /* texData_ is emptied while the other map owns the memory (see
     * SubMap::releaseTiles); upstream indexes it unchecked. */
    if (id < 0 || static_cast<std::size_t>(id) >= texData_.size()) { return nullptr; }
